  ...
  const onSkillStreamArtifact = (resultId: string, artifact: Artifact, content: string) => {
    ...
    if (artifact && artifact.type === 'codeArtifact') {
      ...
      if (
        ...
        // Prioritize artifact.title (generated by skill) over extracted title from content
        const finalTitle = artifact.title || extractedTitle || 'Code Artifact';

        addNode(
          {
            type: artifact.type,
            data: {
              // Use artifact title first (generated by skill), fallback to extracted title
              title: finalTitle,
