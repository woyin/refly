...
const Flow = memo(
  ({ canvasId }: { canvasId: string }) => {
    ...
    const { isPilotOpen, setIsPilotOpen, setActiveSessionId, activeSessionId } =
    ...
    const { showWorkflowRun, setShowWorkflowRun } = useCanvasResourcesPanelStoreShallow(
      (state) => ({
    ...
    const cleanupTemporaryEdges = useCallback(() => {
      ...
      rfInstance.setEdges((edges) => {
        ...
        return edges.filter(
          (edge) => !isTemporaryNode(edge.source) && !isTemporaryNode(edge.target),
        );
      });
    }, [reactFlowInstance]);
